...
// 3. Left Workbench Controls Component
  ...
  return (
    <div class="lg:col-span-6 xl:col-span-5 flex flex-col gap-6">
      ...
      <div class="border border-[var(--border-outline-val)] bg-[var(--bg-surface-alt)] p-1 grid grid-cols-5 text-xs font-bold uppercase font-mono-code">
        <button x-on:click="switchTab('content')" x-bind:class="activeTab === 'content' ? 'bg-[var(--color-primary-val)] text-black font-extrabold shadow-sm' : 'text-[var(--text-on-surface)] hover:bg-black/10 dark:hover:bg-white/10'" class="py-2.5 flex items-center justify-center gap-1 transition-all">
          <i data-lucide="type" class="w-3.5 h-3.5"></i>
          <span>Text</span>
        </button>
        <button x-on:click="switchTab('theme')" x-bind:class="activeTab === 'theme' ? 'bg-[var(--color-primary-val)] text-black font-extrabold shadow-sm' : 'text-[var(--text-on-surface)] hover:bg-black/10 dark:hover:bg-white/10'" class="py-2.5 flex items-center justify-center gap-1 transition-all">
          <i data-lucide="palette" class="w-3.5 h-3.5"></i>
          <span>Theme</span>
        </button>
        <button x-on:click="switchTab('logo')" x-bind:class="activeTab === 'logo' ? 'bg-[var(--color-primary-val)] text-black font-extrabold shadow-sm' : 'text-[var(--text-on-surface)] hover:bg-black/10 dark:hover:bg-white/10'" class="py-2.5 flex items-center justify-center gap-1 transition-all">
          <i data-lucide="image" class="w-3.5 h-3.5"></i>
          <span>Icon</span>
        </button>
        <button x-on:click="switchTab('pattern')" x-bind:class="activeTab === 'pattern' ? 'bg-[var(--color-primary-val)] text-black font-extrabold shadow-sm' : 'text-[var(--text-on-surface)] hover:bg-black/10 dark:hover:bg-white/10'" class="py-2.5 flex items-center justify-center gap-1 transition-all">
          <i data-lucide="grid" class="w-3.5 h-3.5"></i>
          <span>Style</span>
        </button>
        <button x-on:click="switchTab('footer')" x-bind:class="activeTab === 'footer' ? 'bg-[var(--color-primary-val)] text-black font-extrabold shadow-sm' : 'text-[var(--text-on-surface)] hover:bg-black/10 dark:hover:bg-white/10'" class="py-2.5 flex items-center justify-center gap-1 transition-all">
      ...
      <div x-show="activeTab === 'content'" class="border border-[var(--border-outline-val)] bg-[var(--bg-surface-alt)] p-5 space-y-4">
        ...
        <div class="space-y-1">
          ...
          <div class="grid grid-cols-3 gap-2">
            <button 
              x-on:click="align = 'flex-start'" 
              x-bind:class="align === 'flex-start' ? 'bg-[var(--color-primary-val)] text-black font-bold' : 'bg-[var(--bg-surface)] text-[var(--text-on-surface)] hover:border-amber-500'" 
            ...
            <button 
              x-on:click="align = 'center'" 
              x-bind:class="align === 'center' ? 'bg-[var(--color-primary-val)] text-black font-bold' : 'bg-[var(--bg-surface)] text-[var(--text-on-surface)] hover:border-amber-500'" 
              class="py-2 border border-[var(--border-outline-val)] text-xs font-mono-code transition-all flex items-center justify-center gap-1.5">
              <i data-lucide="align-center" class="w-3.5 h-3.5"></i>
              <span>Center</span>
            </button>
            <button 
              x-on:click="align = 'flex-end'" 
              x-bind:class="align === 'flex-end' ? 'bg-[var(--color-primary-val)] text-black font-bold' : 'bg-[var(--bg-surface)] text-[var(--text-on-surface)] hover:border-amber-500'" 
              class="py-2 border border-[var(--border-outline-val)] text-xs font-mono-code transition-all flex items-center justify-center gap-1.5">
      ...
      <div x-show="activeTab === 'theme'" class="border border-[var(--border-outline-val)] bg-[var(--bg-surface-alt)] p-5 space-y-4">
        ...
        <div class="space-y-2">
          ...
          <div class="grid grid-cols-3 sm:grid-cols-4 gap-2.5 max-h-64 overflow-y-auto p-1.5 border border-[var(--border-outline-val)] bg-[var(--bg-surface)]">
            <template x-for="t in themePresets" x-bind:key="t.id">
              <button 
                x-on:click="theme = t.id"
                x-bind:style="'background:' + t.bg"
                x-bind:class="theme === t.id ? 'ring-2 ring-amber-500 scale-[1.02] font-black' : 'opacity-80 hover:opacity-100'"
      ...
      <div x-show="activeTab === 'logo'" class="border border-[var(--border-outline-val)] bg-[var(--bg-surface-alt)] p-5 space-y-4">
        ...
        <div class="grid grid-cols-4 gap-1 border border-[var(--border-outline-val)] p-1 bg-[var(--bg-surface)]">
          <button x-on:click="logoMode = 'preset'" x-bind:class="logoMode === 'preset' ? 'bg-amber-500 text-black font-extrabold' : 'text-[var(--text-on-surface)]'" class="py-1.5 text-[11px] font-mono-code uppercase transition-all">Presets</button>
          <button x-on:click="logoMode = 'url'" x-bind:class="logoMode === 'url' ? 'bg-amber-500 text-black font-extrabold' : 'text-[var(--text-on-surface)]'" class="py-1.5 text-[11px] font-mono-code uppercase transition-all">URL</button>
          <button x-on:click="logoMode = 'upload'" x-bind:class="logoMode === 'upload' ? 'bg-amber-500 text-black font-extrabold' : 'text-[var(--text-on-surface)]'" class="py-1.5 text-[11px] font-mono-code uppercase transition-all">Upload</button>
          <button x-on:click="logoMode = 'none'" x-bind:class="logoMode === 'none' ? 'bg-amber-500 text-black font-extrabold' : 'text-[var(--text-on-surface)]'" class="py-1.5 text-[11px] font-mono-code uppercase transition-all">None</button>
        </div>
        ...
        <div x-show="logoMode === 'preset'" class="space-y-3">
          ...
          <template x-if="validTheSvgLogo">
            <div class="p-3 border border-amber-500/50 bg-amber-500/10 flex items-center justify-between gap-3">
              <div class="flex items-center gap-3 overflow-hidden">
                <img 
                  x-bind:src="'https://cdn.jsdelivr.net/gh/glincker/thesvg@main/public/icons/' + validTheSvgLogo + '/default.svg'" 
              ...
              <button 
                x-on:click="presetLogo = validTheSvgLogo"
                x-bind:class="presetLogo === validTheSvgLogo ? 'bg-amber-500 text-black font-extrabold' : 'border border-amber-500/40 text-amber-400 hover:bg-amber-500 hover:text-black'"
          ...
          <div class="grid grid-cols-4 sm:grid-cols-5 gap-2 max-h-48 overflow-y-auto p-1.5 border border-[var(--border-outline-val)] bg-[var(--bg-surface)]">
            <template x-for="icon in filteredIcons" x-bind:key="icon.id">
              <button 
                x-on:click="presetLogo = icon.id"
                x-bind:class="presetLogo === icon.id ? 'border-amber-500 bg-amber-500/20 text-white font-bold ring-1 ring-amber-500' : 'border-[var(--border-outline-val)] hover:border-amber-500/60 text-[var(--text-on-surface)]'"
          ...
          <div class="space-y-1">
            ...
            <div class="flex flex-wrap gap-1">
              <template x-for="slug in ['linear', 'clerk', 'resend', 'prisma', 'drizzle', 'hono', 'deno', 'bun', 'astro', 'nuxt', 'remix', 'firebase', 'redis', 'postgresql', 'mongodb']" x-bind:key="slug">
                <button 
                  x-on:click="presetLogo = slug; iconSearch = slug"
                  x-bind:class="presetLogo === slug ? 'bg-amber-500 text-black font-bold' : 'bg-[var(--bg-surface)] text-[var(--text-on-surface)] border border-[var(--border-outline-val)] hover:border-amber-500'"
      ...
      <div x-show="activeTab === 'pattern'" class="border border-[var(--border-outline-val)] bg-[var(--bg-surface-alt)] p-5 space-y-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5 max-h-60 overflow-y-auto p-1.5 border border-[var(--border-outline-val)] bg-[var(--bg-surface)]">
          <template x-for="p in patterns" x-bind:key="p.id">
            <button 
              x-on:click="pattern = p.id"
              x-bind:class="pattern === p.id ? 'bg-amber-500 text-black font-extrabold border-black' : 'bg-[var(--bg-surface)] text-[var(--text-on-surface)] hover:border-amber-500'"
...
// 4. Right Live Preview Component
      ...
      <div class="border border-[var(--border-outline-val)] bg-[var(--bg-surface-alt)] p-5 space-y-4 sticky top-20">
        ...
        <div class="relative w-full bg-black border border-[var(--border-outline-val)] overflow-hidden min-h-[300px] flex items-center justify-center shadow-2xl">
          ...
          <img 
            ...
            x-bind:class="loading ? 'opacity-40' : 'opacity-100'"
          />
        </div>
        ...
        <div class="border-t border-[var(--border-outline-val)] pt-4 space-y-3">
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-center bg-[var(--bg-surface)] border border-[var(--border-outline-val)] p-1.5 text-xs font-mono-code">
              ...
              <button 
                x-on:click="copyToClipboard(fullApiUrl, 'Copied Direct Image URL!')"
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-center bg-[var(--bg-surface)] border border-[var(--border-outline-val)] p-1.5 text-xs font-mono-code">
              ...
              <button 
                x-on:click="copyToClipboard(markdownSnippet, 'Copied Markdown snippet!')"
                class="px-3 py-1 bg-amber-500/20 hover:bg-amber-500 hover:text-black text-amber-400 font-bold border border-amber-500/40 uppercase transition-all flex items-center gap-1">
                <i data-lucide="copy" class="w-3.5 h-3.5"></i>
                <span>Copy</span>
              </button>
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-center bg-[var(--bg-surface)] border border-[var(--border-outline-val)] p-1.5 text-xs font-mono-code">
              ...
              <button 
                x-on:click="copyToClipboard(htmlSnippet, 'Copied HTML tag!')"
                class="px-3 py-1 bg-amber-500/20 hover:bg-amber-500 hover:text-black text-amber-400 font-bold border border-amber-500/40 uppercase transition-all flex items-center gap-1">
                <i data-lucide="copy" class="w-3.5 h-3.5"></i>
                <span>Copy</span>
              </button>
  ...
  const reasons = [
    {
      ...
      desc: 'Powered by Cloudflare Workers & next-gen Satori engine. Banners are rendered dynamically at edge nodes in milliseconds.',
      icon: 'zap',
      color: 'amber'
    },
    {
      num: '02. FREE FOREVER',
      title: '100% Free & No API Key',
      desc: 'Completely free for personal projects, blogs, open-source repos, and commercial sites without signups or API keys.',
      icon: 'gift',
      color: 'emerald'
    },
    {
      num: '03. RICH ICONS',
      title: '6,400+ Tech Icons & Logos',
      desc: 'Seamless integration with local tech brand vector collection and real-time validation with glincker/thesvg repository.',
      icon: 'shapes',
      color: 'sky'
    },
    {
      num: '04. MULTI-PLATFORM',
      title: 'Multi-Platform Preset Sizes',
      desc: 'Pre-configured dimensions for Facebook Open Graph (1200x630), Twitter/X (1200x600), YouTube Covers (1280x720), and Instagram (1080x1080).',
      icon: 'layout-grid',
      color: 'purple'
    },
    {
      num: '05. STYLING SYSTEM',
      title: '24+ Color Schemes & Patterns',
      desc: 'Choose from 24 curated gradient palettes (Flat UI, Aussie, British) matched with 20+ patterns like Glow Mesh, Circuit, and Hexagons.',
      icon: 'palette',
      color: 'pink'
    },
    {
      num: '06. DYNAMIC API',
      title: 'Direct Dynamic URL API',
      desc: 'Simply embed dynamic URL endpoints into og:image meta tags or Markdown README files without hosting static files.',
      icon: 'code-2',
      color: 'amber'
    },
    {
      num: '07. FLEXIBLE CONTROLS',
      title: 'Top-to-Bottom Customization',
      desc: 'Fine-tune title sizes, font alignment (Left/Center/Right), logo shapes (Round/Rounded/Square), and custom brand watermarks.',
      icon: 'sliders',
      color: 'indigo'
    },
    {
      num: '08. MODERN PWA',
      title: 'Modern Responsive UI & PWA',
      desc: 'Enjoy a sleek Penguin UI interface with Dark/Light mode support, LocalStorage persistence, and PWA desktop/mobile installation.',
      icon: 'smartphone',
      color: 'teal'
    }
  ]
    ...
    <section class="border-t border-[var(--border-outline-val)] bg-[var(--bg-surface-alt)] py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div class="max-w-7xl mx-auto space-y-8 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {reasons.map((r) => (
            <div key={r.num} class="bg-[var(--bg-surface)] p-5 border border-[var(--border-outline-val)] hover:border-amber-500/60 transition-all duration-200 group flex flex-col justify-between">
              <div class="space-y-3">
                ...
                <div class="space-y-1">
                  <span class="text-[10px] text-amber-500 font-bold uppercase tracking-widest font-mono-code">{r.num}</span>
                  <h3 class="text-sm font-extrabold text-[var(--text-on-surface-strong)] tracking-tight">{r.title}</h3>
                </div>
                <p class="text-xs text-[var(--text-on-surface)] leading-relaxed opacity-85">{r.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-6 border-b border-[var(--border-outline-val)] text-xs">
          <div class="flex flex-wrap items-center gap-x-3 gap-y-2 text-[var(--text-on-surface)]">
            ...
            <a href="https://workers.cloudflare.com/" target="_blank" rel="noopener noreferrer" class="hover:text-amber-500 transition-colors flex items-center gap-1 text-[var(--text-on-surface)]">
              <i data-lucide="zap" class="w-3 h-3 text-amber-500"></i>
              <span>Cloudflare Workers</span>
            </a>
            <span class="text-[var(--border-outline-val)]">•</span>
            <a href="https://flatuicolors.com/" target="_blank" rel="noopener noreferrer" class="hover:text-amber-500 transition-colors flex items-center gap-1 text-[var(--text-on-surface)]">
              <i data-lucide="palette" class="w-3 h-3 text-amber-500"></i>
              <span>Flat UI Colors</span>
            </a>
            <span class="text-[var(--border-outline-val)]">•</span>
            <a href="https://thesvg.org/" target="_blank" rel="noopener noreferrer" class="hover:text-amber-500 transition-colors flex items-center gap-1 text-[var(--text-on-surface)]">
              <i data-lucide="image" class="w-3 h-3 text-amber-500"></i>
              <span>theSVG Icon Library</span>
            </a>
            <span class="text-[var(--border-outline-val)]">•</span>
            <a href="https://www.penguinui.com" target="_blank" rel="noopener noreferrer" class="hover:text-amber-500 transition-colors flex items-center gap-1 text-[var(--text-on-surface)]">
          ...
          <div class="flex items-center gap-2">
            ...
            <a href="https://fcode.vn" target="_blank" rel="noopener noreferrer" class="font-black text-amber-500 hover:text-black hover:bg-amber-500 transition-all flex items-center gap-1.5 bg-amber-500/10 px-2.5 py-1 border border-amber-500/40 text-xs uppercase tracking-wider">
              <span>fcode.vn</span>
              <i data-lucide="external-link" class="w-3 h-3"></i>
            </a>
          </div>
...
// Main Page JSX Component
export function FrontendPage({ gaId }: FrontendProps) {
  ...
  return (
    <html lang="en" class="dark">
      <head>
        ...
        <style type="text/tailwindcss">{raw(`
          @theme {
